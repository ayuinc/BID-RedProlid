...
function wppb_signup_user_notification( $user, $user_email, $activation_key, $meta = '' ) {
	...
	$message = sprintf( 
		apply_filters('wppb_signup_user_notification_email_content', 
			__("Hola,<br><br>Para activar tu cuenta de usuaria o usuario, por favor haz clic en el enlace (final de este mensaje)..\n\nInmediatamente después de hacerlo recibirás un nuevo correo electrónico con tu nombre de usuaria o usuario.<br><br>Si tienes algún problema por favor escríbenos a: comunicate@redprolid.org Muchas gracias,<br><br>Equipo de Red PROLID.<br><br> %s[ACTIVA TU CUENTA]", 
				'profilebuilder' ), 
			$user_email, 
			$user, 
			$activation_key, 
...
function wppb_notify_user_registration_email( $bloginfo, $user_name, $email, $send_credentials_via_email, $password, $adminApproval ){
	...
	if ( isset( $send_credentials_via_email ) && ( $send_credentials_via_email == 'sending' ) ){
		...
		$user_message_content = sprintf( __( '¡Bienvenida o bienvenido a Red PROLID!<br/><br/>Tu cuenta ha sido activa correctamente. Ingresa con tu nombre de usuario(a) y contraseña a través de este enlace: http://redprolid.org', 'profilebuilder' ), $user_message_from, $user_name, $password );
