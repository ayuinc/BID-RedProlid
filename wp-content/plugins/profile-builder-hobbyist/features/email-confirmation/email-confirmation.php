...
function wppb_signup_user_notification( $user, $user_email, $activation_key, $meta = '' ) {
	...
	$message = sprintf( apply_filters( 'wppb_signup_user_notification_email_content', __( "Hola, \nPara activar tu cuenta de usuaria o usuario, por favor haz clic en el siguiente enlace:\n\n %s \n\nInmediatamente después de hacerlo recibirás un nuevo correo electrónico con tu nombre de usuaria o usuario. \n\nSi tienes algún problema por favor escríbenos a: comunícate@redprolid.org \n\nMuchas gracias, \n\nEquipo de Red PROLID.\n\n", "profilebuilder" ), $user_email, $user, $activation_key, $registration_page_url, $meta, $from_name, 'wppb_user_emailc_registr_w_email_confirm_email_content' ), '<a href="'.$registration_page_url.'">', $registration_page_url, '</a>.' );
...
function wppb_notify_user_registration_email( $bloginfo, $user_name, $email, $send_credentials_via_email, $password, $adminApproval ){
	...
	if ( isset( $send_credentials_via_email ) && ( $send_credentials_via_email == 'sending' ) ){
		...
		$user_message_content = sprintf( __( 'Bienvenido a %1$s!<br/><br/><br/>Tu nombre de ususario es:%2$s y tu contraseña:%3$s', 'profilebuilder' ), $user_message_from, $user_name, $password );
